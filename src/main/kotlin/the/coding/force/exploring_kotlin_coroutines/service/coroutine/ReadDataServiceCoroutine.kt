...
class ReadDataServiceCoroutine(
     ...
     * if the data exists it will be get in database, case the data does not exist an exception
     * DataNotFoundException` will be thrown. The `logger` will register the status of read operation
     *
     * @author Lucas Terra
     * @param dataId ID from `DataEntity` received from suspend controller `read` to find in database
     * @return `ReadDataResponse` case a register exists in database
